...
export default function ComplaintLetter({ pins, reports = [], onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      { signal: controller.signal, headers: { "User-Agent": `Noisecatcher/1.0 ${SITE_DOMAIN}` } }
    )
...
MEASUREMENT DATA
Recorded using Noisecatcher (${SITE_URL}), a mobile acoustic measurement application.
...
${SITE_URL} · All readings are relative to device microphone, not calibrated SPL.`;
